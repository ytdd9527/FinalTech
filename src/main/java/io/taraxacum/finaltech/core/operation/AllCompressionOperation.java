 ...
 * @author Final_ROOT
 */
public interface AllCompressionOperation extends MachineOperation {
    int COPY_CARD = 1;
    int ITEM_FAKE = 2;
    int ERROR_ITEM = -1;
    ...
    static int getType(@Nonnull ItemStack item) {
        ...
        if(ItemStackUtil.isItemSimilar(item, FinalTechItems.SINGULARITY) || ItemStackUtil.isItemSimilar(item, FinalTechItems.SPIROCHETE) || ItemStackUtil.isItemSimilar(item, FinalTechItems.FAKE)) {
            return ERROR_ITEM;
        }
        return COPY_CARD;
    }
    ...
    @Nullable
    static AllCompressionOperation newInstance(@Nonnull ItemStack item) {
