 ...
 * @since 2.0
 */
public class EnergizedOperationAccelerator extends AbstractFaceMachine implements RecipeItem, EnergyNetComponent, MenuUpdater, LocationMachine {
            ...
            if(locationInfo != null && !this.notAllowedId.contains(locationInfo.getId())) {
                if (locationInfo.getSlimefunItem() instanceof MachineProcessHolder machineProcessHolder && locationInfo.getSlimefunItem() instanceof EnergyNetComponent energyNetComponent) {
                    ...
                    Runnable runnable = () -> {
                        ...
                        if (operation != null) {
                            int time = Math.min(Math.min(amount * this.baseEfficiency + FinalTech.getRandom().nextInt(amount * this.randomEfficiency), energy / energyNetComponent.getCapacity()), operation.getRemainingTicks());
                            if(time > 0) {
