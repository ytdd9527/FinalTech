...
public final class StringNumberUtil {
     ...
     * 第一个数较大时，返回true
     * 第二个数较大时，返回false
     * 临时使用，后续请替换为Compare(String, String)方法
    ...
    private static String easilyAdd(String stringNumber1, String stringNumber2) {
        ...
        for(i = 0; i < minLength; i++) {
            r = s1[s1.length - i - 1] + s2[s2.length - i - 1] + add - 96;
            add = r / 10;
            r %= 10;
        ...
        int n2;
        for(; i < maxLength; i++) {
            n1 = s1.length > i ? s1[s1.length - i - 1] : 48;
            n2 = s2.length > i ? s2[s2.length - i - 1] : 48;
            r = n1 + n2 + add - 96;
            add = r / 10;
            r %= 10;
            stringBuilder.append(r);
