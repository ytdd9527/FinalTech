...
public class AdvancedCraftUtil {
    ...
    public MachineRecipe calEnlargeMachineRecipe() {
        ...
        ItemStack[] inputItems = ItemStackUtil.enlargeItemArray(this.machineRecipe.getInput(), this.matchCount);
        ItemStack[] outputItems = ItemStackUtil.enlargeItemArray(this.machineRecipe.getOutput(), this.matchCount);
    ...
    public static AdvancedCraftUtil calCraft(BlockMenu blockMenu, int[] slots, List<MachineRecipe> machineRecipeList, int quantityModule, int offset) {
        Map<Integer, ItemStackWithWrapper> itemSlotMap = MachineUtil.parseItemWithSlot(blockMenu.toInventory(), slots);
