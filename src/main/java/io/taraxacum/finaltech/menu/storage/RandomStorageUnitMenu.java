...
public class RandomStorageUnitMenu extends AbstractMachineMenu {
    ...
    public int[] getSlotsAccessedByItemTransport(ItemTransportFlow itemTransportFlow) {
        ...
        int[] selectSlots = new int[0];
        if(ItemTransportFlow.INSERT.equals(itemTransportFlow)) {
            selectSlots = INPUT_SLOT;
        } else if(ItemTransportFlow.WITHDRAW.equals(itemTransportFlow)) {
            selectSlots = OUTPUT_SLOT;
        }

        List<Integer> collect = Arrays.stream(selectSlots).boxed().collect(Collectors.toList());
        Collections.shuffle(collect);
        int[] resultSlots = new int[selectSlots.length];
        for(int i = 0; i < collect.size(); i++) {
            resultSlots[i] = collect.get(i);
        }
        return resultSlots;
    }
