...
public class RandomInputStorageUnitMenu extends AbstractMachineMenu {
    ...
    public RandomInputStorageUnitMenu(@Nonnull AbstractMachine machine) {
        super(machine);
    }
    ...
    public int[] getSlotsAccessedByItemTransport(ItemTransportFlow itemTransportFlow) {
        if(ItemTransportFlow.WITHDRAW.equals(itemTransportFlow)) {
            return OUTPUT_SLOT;
        } else if(itemTransportFlow == null) {
            return new int[0];
        }
        List<Integer> collect = Arrays.stream(INPUT_SLOT).boxed().collect(Collectors.toList());
        Collections.shuffle(collect);
        int[] inputSlots = new int[INPUT_SLOT.length];
