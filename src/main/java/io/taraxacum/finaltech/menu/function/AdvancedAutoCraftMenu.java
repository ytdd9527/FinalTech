...
import io.taraxacum.finaltech.factory.AdvancedAutoCraftLocationRegistry;
...
public class AdvancedAutoCraftMenu extends AbstractMachineMenu {
    ...
    public void init() {
        ...
        this.addItem(INPUT_SEARCH_SLOT, SlotSearchSize.OUTPUT_HELPER.getDefaultIcon());
        this.addMenuClickHandler(INPUT_SEARCH_SLOT, ChestMenuUtils.getEmptyClickHandler());
        this.addItem(OUTPUT_SEARCH_SLOT, SlotSearchSize.INPUT_HELPER.getDefaultIcon());
    ...
    public void updateMenu(@Nonnull BlockMenu blockMenu, @Nonnull Block block) {
        ...
        String slotSearchSizeInputValue = SlotSearchSize.INPUT_HELPER.getValue(location);
        blockMenu.replaceExistingItem(INPUT_SEARCH_SLOT, SlotSearchSize.INPUT_HELPER.getIcon(slotSearchSizeInputValue));
        SlotSearchSize.OUTPUT_HELPER.checkOrSetBlockStorage(location);
        String slotSearchSizeOutputValue = SlotSearchSize.OUTPUT_HELPER.getValue(location);
        blockMenu.replaceExistingItem(OUTPUT_SEARCH_SLOT, SlotSearchSize.OUTPUT_HELPER.getIcon(slotSearchSizeOutputValue));
        ...
        if (ItemStackUtil.isItemNull(parseItem)) {
            ...
            if (ItemStackUtil.isItemNull(parseItem) || ItemStackUtil.isItemSimilar(parseItem, Icon.PARSE_FAILED_ICON)) {
    ...
    private void setParseFailedMenu(@Nonnull BlockMenu blockMenu) {
        AdvancedAutoCraftLocationRegistry.setRecipe(blockMenu.getLocation(), null);
        for (int slot : ITEM_INPUT_SLOT) {
    ...
    private void setParseSuccessMenu(@Nonnull BlockMenu blockMenu, @Nonnull AdvancedMachineRecipe advancedMachineRecipe) {
        AdvancedAutoCraftLocationRegistry.setRecipe(blockMenu.getLocation(), advancedMachineRecipe);
        int i;
