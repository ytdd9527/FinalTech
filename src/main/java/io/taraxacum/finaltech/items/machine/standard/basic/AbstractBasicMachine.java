...
import io.taraxacum.finaltech.items.machine.standard.AbstractStandardMachine;
import io.taraxacum.finaltech.menu.AbstractMachineMenu;
import io.taraxacum.finaltech.menu.standard.lock.AbstractLockMachineMenu;
import io.taraxacum.finaltech.menu.standard.lock.BasicMachineMenu;
    ...
    protected final MachineRecipe matchRecipe(@Nonnull BlockMenu blockMenu, int offset, int recipeLock) {
        ...
        if(craft != null) {
            ...
            if(craft.getMatchCount() > 0) {
                ...
                if(recipeLock == Integer.parseInt(RecipeLockHelper.VALUE_UNLOCK)) {
                    ItemStack item = blockMenu.getItemInSlot(AbstractLockMachineMenu.RECIPE_LOCK_SLOT);
                    RecipeLockHelper.HELPER.setIcon(item, String.valueOf(craft.getOffset()), this);
                    BlockStorage.addBlockInfo(blockMenu.getLocation(), RecipeLockHelper.KEY, String.valueOf(craft.getOffset()));
                } else if(recipeLock == Integer.parseInt(RecipeLockHelper.VALUE_LOCK_OFF)) {
