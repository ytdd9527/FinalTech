...
public class TemplateParser {
     ...
     * includes:
     *      all items should be crafted by at least one FINALTECH_ANNULAR
    ...
    public void registerMachine() {
        ...
        for(String id : idList) {
            ...
            try {
                ...
                name = FinalTech.getLanguageManager().getResult(name);
                ItemStackUtil.setItemName(itemStack, name);

                if(this.configFile.containPath(id, "lore")) {
                    ItemStackUtil.setLore(itemStack, FinalTech.getLanguageManager().getResult(this.configFile.getStringList(id, "lore")));
                }
                ...
                switch (type) {
                    case "BASIC" -> basicMachineList.add(this.registerBasicMachine(slimefunItemStack, recipeType, craftRecipeItems, machineRecipeList));
                    case "ADVANCED" -> advanceMachineList.add(this.registerAdvancedMachine(slimefunItemStack, recipeType, craftRecipeItems, machineRecipeList));
                    default -> throw new TemplateParserErrorException("Template file parse field. Can not register machine with type '" + type + "' !!!");
                }
                FinalTech.logger().info("Slimefun item " + id + " is registered from template");

                if(this.configFile.containPath(id, "research")) {

                }
                new Research(new NamespacedKey(FinalTech.getInstance(), "key"), 0, "name", 0);
            } catch (Exception e) {
                FinalTech.logger().severe("An error occurred while registering Slimefun item [" + id + "] from template file");
                if(e instanceof TemplateParserErrorException templateParserErrorException) {
                    FinalTech.logger().severe(templateParserErrorException.getMessage());
                }
                e.printStackTrace();
            }
        }
